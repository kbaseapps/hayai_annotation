package hayai_annotation::hayai_annotationClient;
...
hayai_annotation::hayai_annotationClient
...
sub new
{
    ...
    my $self = {
	client => hayai_annotation::hayai_annotationClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
AnnotatePlantGenomeParams is a reference to a hash where the following keys are defined:
	...
	organism_type has a value which is a string
	max_hits_per_query has a value which is an int
	max_seq_id has a value which is a float
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "hayai_annotation.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "hayai_annotation.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for hayai_annotation::hayai_annotationClient\n";
    }
    if ($sMajor == 0) {
        warn "hayai_annotation::hayai_annotationClient version is $svr_version. API subject to change.\n";
    }
}
...
package hayai_annotation::hayai_annotationClient::RpcClient;
